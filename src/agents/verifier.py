...
class VerifierAgent:
    """Specialist agent that independently verifies candidate output schemas and entity integrity."""

    def __init__(self, repo=None, data_dir: str = "data"):
        self.repo = repo
        self.datastore = CSVDataStore(data_dir=data_dir)

    def verify(self, case_id: str, output: Dict[str, Any], check_csv: bool = False) -> ValidationReport:
        errors_str = validate_output(output, check_csv=check_csv)
        status = "passed" if not errors_str else "failed"
        error_dicts = [{"message": err} for err in errors_str]
