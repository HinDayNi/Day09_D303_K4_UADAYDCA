...
class ResultAssembler:
    ...
    def assemble(
        ...
        has_items = bool(ord_prod.get("items", []))

        # ── Extract policy_data fields ──
        ...
        confidence = case_assess.get("confidence") if case_assess.get("confidence") is not None else policy_data.get("confidence", 0.95)
        ...
        seller_ids = ord_prod.get("sellers", [])[:3] if has_items else []
        payment_ids = (pay.get("affected_payment_ids") or pay.get("payment_ids", []))[:5]

        # ── 2. Customer context (scope-aware) ──
        ...
        if include_customer_history:
            related_order_ids = (
                cust_ctx.get("related_order_ids")
                if "related_order_ids" in cust_ctx
                else cust.get("related_order_ids", [])
            )[:5]
        else:
            related_order_ids = []

        # ── 3. Product context (scope-aware) ──
        prod_ctx = cust.get("product_context", {}) if isinstance(cust.get("product_context"), dict) else {}
        if has_items and include_product_context:
        ...
        # ── 4. Delivery analysis ──
        order_status = ord_prod.get("order_status")
        if order_status in ("canceled", "unavailable"):
            ...
            seller_handoff_analysis = deliv.get("seller_handoff_analysis", []) if has_items else []
            late_handoff_seller_ids = deliv.get("late_handoff_seller_ids", []) if has_items else []
        ...
        # ── 6. Evidence IDs (strictly aligned with responsible_parties) ──
        ...
        # Only add seller evidence for sellers in responsible_parties
        responsible_seller_ids = set()
        for party in responsible_parties:
            if party.get("party_type") == "seller":
                s_id = party.get("party_id")
                if s_id and s_id not in ("OLIST_PLATFORM", "LOGISTICS_PROVIDER"):
                    responsible_seller_ids.add(s_id)
        for s_id in responsible_seller_ids:
        ...
        # Deduplicate preserving order and cap at 20
        seen: set[str] = set()
